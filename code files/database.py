...
def init_db():
    ...
    try:
        ...
        pass
    try:
        cur.execute("ALTER TABLE player ADD COLUMN volume REAL DEFAULT 1.0")
    except sqlite3.OperationalError:
        pass

    cur.execute("SELECT COUNT(*) FROM player")
    if cur.fetchone()[0] == 0:
        cur.execute("INSERT INTO player (coins, active_skin, death_screen_enabled, volume) VALUES (?, ?, ?, ?)", (0, "p1", 1, 1.0))

    cur.execute("SELECT COUNT(*) FROM skins")
    if cur.fetchone()[0] == 0:
        cur.execute("INSERT INTO skins (name, unlocked, price) VALUES (?, ?, ?)", ("p1", 1, 0))
        cur.execute("INSERT INTO skins (name, unlocked, price) VALUES (?, ?, ?)", ("p2", 0, 50))
        cur.execute("INSERT INTO skins (name, unlocked, price) VALUES (?, ?, ?)", ("p3", 0, 50))
...
def get_player_data():
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT coins, active_skin, death_screen_enabled, volume FROM player WHERE id=1")
    row = cur.fetchone()
    conn.close()
    return {"coins": row[0], "active_skin": row[1], "death_screen_enabled": bool(row[2]), "volume": row[3]}
...
def set_volume(vol):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("UPDATE player SET volume = ? WHERE id=1", (vol,))
    conn.commit()
    conn.close()